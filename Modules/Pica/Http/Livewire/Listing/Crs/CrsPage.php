...
class CrsPage extends Component
{
    ...
    public function mount()
    {
        ...
        $this->fieldSource = PicaDocument::where('published', PicaStatus::Publish)
            ->where('requested', PicaStatus::RequestedCrs)
            ->get()
            ->groupBy('source')
            ->toBase();
    ...
    public function removeItem()
    {
        foreach ($this->itemSelected as $item) {
            $pica = PicaDocument::find($item);

            $pica->picaFiles()->delete();

            $pica->activities()->delete();
    ...
    {
        return view('pica::livewire.listing.crs.crs-page')->layout('pica::layouts.app');
    }
}
