...
namespace Modules\Pica\Http\Livewire\Listing\Draft;
...
class DraftPage extends Component
{
    ...
    {
        $pica = PicaDocument::with('pica')->where('published', PicaStatus::Draft)->get();
        ...
        $this->fieldSource = PicaDocument::where('published', PicaStatus::Draft)
            ->get()
            ->groupBy('source')
            ->toBase();

        $this->fieldCompany = PicaDocument::where('published', PicaStatus::Draft)
            ->get()
            ->groupBy('company_id')
        ...
        $this->fieldLocation = PicaDocument::where('published', PicaStatus::Draft)
            ->get()
            ->groupBy('location_id')
            ->map(function ($item, $key) {
        ...
        $this->fieldStatus = PicaDocument::where('published', PicaStatus::Draft)
            ->get()
            ->groupBy('status')
            ->toBase();
    ...
    public function getActiveListingsProperty(): LengthAwarePaginator
    {
        return PicaDocument::when(!empty($this->sortSelected), function ($query) {
            ...
            ->where('published', PicaStatus::Draft)
    ...
    {
        return view('pica::livewire.listing.draft.draft-page')->layout('pica::layouts.app');
    }
}
