...
<div class="inner-content">
    ...
    <div class="section-content">
        ...
        <div class="py-3 px-2 flex gap-4">
            <div class="section-title ">
                <p class="text-xl text-black">Bowtie Lost Calculation</p>
            </div>
            ...
            <div x-data="{ itemSelected: @entangle('itemSelected'), info: @entangle('info') }">
                ...
                <div class="toolbar-tables border-top border-bottom d-flex justify-content-between p-2 sticky-top">
                    ...
                    <div class="toolbar-left d-flex align-items-center">
                        <button onclick="tryToOpenModalLostCallculation()" type="button" class="button-toolbar d-flex gap-2 align-items-center py-2 px-3">
                            ...
                            <tbody>
                                @if (count($data) > 0)
                                    @foreach ($data as $itemIndex => $item)
                                        <tr>
                                            <td wire:click.prevent="onSelectedItem('{{ $item->id }}')">{{ $item->event->name ?? '-'}}</td>
                                            <td>
                                            ...
                                            <td style="text-align: right;">
                                                @foreach($item->details as $key => $detail)
                                                    {{  "Rp " . number_format($detail->amount, 0, ',', '.') }}<br>
                                                @endforeach
                                                <hr>
                                                <b>{{  "Rp " . number_format($item->details_sum_amount, 0, ',', '.') }}</b>
                                            </td>
                                            <td style="text-align: right;">
                                                @foreach($item->details as $key => $detail)
                                                    {{  "$ " . number_format($detail->amount / 14000, 2) }}<br>
                                                @endforeach
                                                <hr>
                                                <b>{{  "$ " . number_format($item->details_sum_amount / 14000, 2) }}</b>
                                            </td>
                                            <!-- <td wire:click.prevent="onSelectedItem('{{ $item->id }}')">{{  "Rp " . number_format($item->details_sum_amount, 0, ',', '.') }}</td>
                                            <td wire:click.prevent="onSelectedItem('{{ $item->id }}')">{{  "$ " . number_format($item->details_sum_amount / 14000, 2) }}</td> -->

                                            <td>
                                                <button type="button" onclick="openModalEditLostCallculation('{{ $item->id }}')" class="px-3 py-2 bg-green-500 rounded-md text-white duration-300 hover:bg-green-700" onclick="clickEditModalFromList('{{ $item->id }}')">
                                                    Edit
                                                </button>
                                            </td>
                                        </tr>
    ...
    <script>
        ...
        $('#modal_loss_calculation').on('hide.bs.modal', function (e) {
            Livewire.emit('check_lost_callculation');
        });
    
       
        function tryToOpenModalLostCallculation() {
            Livewire.emit('clear_lost_callculation_modal')
