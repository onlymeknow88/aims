    ...
    <div class="section-content">
        ...
        <div class="py-3 px-2 flex gap-4">
            <div class="section-title ">
                <p class="text-xl text-black">Bowtie CCA List</p>
            </div>
            ...
            <div x-data="{ itemSelected: @entangle('itemSelected'), info: @entangle('info') }">
                ...
                <div class="toolbar-tables border-top border-bottom d-flex justify-content-between p-2 sticky-top">
                    ...
                    <div class="toolbar-left d-flex align-items-center">
                        <button onclick="tryToOpenModalCca()" type="button"
                            ...
                            <tbody>
                                @if (count($data) > 0)
                                    @foreach ($data as $itemIndex => $item)
                                        <tr>
                                            <td wire:click.prevent="onSelectedItem('{{ $item->id }}')">
                                                {{ $item->control_objectives }}</td>
                                            <td wire:click.prevent="onSelectedItem('{{ $item->id }}')">
                                            ...
                                            <td wire:click.prevent="onSelectedItem('{{ $item->id }}')">
                                                {{ $item->step_seven }}</td>
                                            <td wire:click.prevent="onSelectedItem('{{ $item->id }}')">
                                                {{ $item->control_regulation }}</td>
                                            <td wire:click.prevent="onSelectedItem('{{ $item->id }}')">
                                                {{ $item->number }}</td>
                                            <td>
                                                <button type="button"
                                                    onclick="openModalEditCca('{{ $item->id }}')"
                                                    class="px-3 py-2 bg-green-500 rounded-md text-white duration-300 hover:bg-green-700"
                                                    onclick="clickEditModalFromList('{{ $item->id }}')">
                                                    Edit
                                                </button>
                                            </td>
                                        </tr>
                                            ...
                                            <table id="multi-sort" class="table">
                                                ...
                                                <tbody>
                                                    @foreach ($columns as $column => $detail)
                                                        @include(
                                                            'livewire.document-systems.maker.components.table-filter',
                                                            [
                                                                'detail' => $detail,
                                                                'column' => $column,
                                                            ]
    ...
    @push('scripts')
        ...
        <script>
            ...
            function tryToOpenModalCca() {
                Livewire.emit('clear_state_cca')
                $("#modal_cca").modal("show");
            }

            function openModalEditCca(id) {
                Livewire.emit('click_edit_cca', id)
            }
        </script>
    @endpush
