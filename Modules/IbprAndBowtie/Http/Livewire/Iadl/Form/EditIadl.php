...
class EditIadl extends Component
{
    protected $listeners = [
    ...
    public function mount($id){
        $this->iadl_id = $id;
        $this->iadl = Iadl::with(['teams', 'forms'])->find($id);

        $this->ccow = $this->ccow = Company::where('type', CompanyType::Internal()->value)->get();
        $this->pja = User::get();
        $this->pjo = User::get();
        $this->pjs = User::get();
        $this->users = User::get();
        ...
        $this->contractors = Company::where('type', CompanyType::Contractor()->value)->where('parent_company_id', $this->iadl->ccow_id)->get();
        $this->sub_contractors = Company::where('type', CompanyType::SubContractor()->value)->where('parent_company_id', $this->iadl->contractor_id)->get();

        $this->ccow_id = $this->iadl->ccow_id;
        $this->iup = $this->iadl->iup;
        $this->department_id = $this->iadl->department_id;
        $this->kriteria = $this->iadl->kriteria;
        $this->contractor_id = $this->iadl->contractor_id;
        $this->sub_contractor_id = $this->iadl->sub_contractor_id;
        $this->pja_id = $this->iadl->pja_id;
        $this->teams = array_column($this->iadl->teams->toArray(), 'user_name');
        $this->team_names = implode(',', $this->teams);
        $this->request_date = $this->iadl->request_date;
        $this->next_date = $this->iadl->next_date;
        $this->document_no = $this->iadl->document_no;
        $this->section_id = $this->iadl->section_id;
        $this->form = $this->iadl->forms;
    }
    ...
    public function goto_list_iadl(Request $request){
       ...
       try {
        ...
        IadlTeam::where('iadl_id', $this->iadl_id)->delete();
    ...
    public function save_ibpr(){
        ...
        // remove old teams;
        return redirect()->route('ibpr-and-bowtie::iadl.active.detail-active-iadl-and-bowtie', [$this->iadl->id]);
    }
    ...
    public function render()
    {
        ...
        return view('ibprandbowtie::livewire.ibpr-and-bowtie.iadl.form.form-edit')->extends('ibprandbowtie::layouts.no-header');
    }
}
