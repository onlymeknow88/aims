...
namespace Modules\FieldLeadership\Http\Livewire\MasterLibrary\TypeKtaTta\Partials;
...
class TableMaker extends Component
{
    ...
    {
        $last = FieldLeadershipKtaAndTta::latest()->first();
        $this->latestUpdate = 'Update on ' . Carbon::parse($last->created_at ?? null)->format('F d, Y . H:i A');

        $this->countData = FieldLeadershipKtaAndTta::get()->count();
    ...
    {
        $type = FieldLeadershipKtaAndTta::find($id);
    ...
    public function removeItem()
    {
        foreach ($this->itemSelected as $item) {
            $type = FieldLeadershipKtaAndTta::find($item);
            $type->delete();
        }
    ...
    {
        return view('fieldleadership::livewire.master-library.type-kta-tta.partials.table-maker')->layout('fieldleadership::layouts.app');
    }
}
