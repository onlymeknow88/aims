<div x-data="{
    ...
    toggleSelectAll() {
        if (this.isAllSelected()) {
            ...
            this.itemSelected = {{ json_encode($this->types->pluck('id')->map(fn($id) => (string)$id)->toArray()) }};
        }
    },
    isAllSelected() {
        return this.itemSelected.length > 0 && this.itemSelected.length === {{ $this->types->count() }};
    },
    toggleItem(id) {
        id = String(id);
    ...
    <div class="table-content table-responsive position-relative">
        ...
        <div class="table-wrapper overflow-auto">
            ...
            <table class="table">
                ...
                <tbody>
                    @foreach ($this->types as $itemIndex => $items)
                        <tr wire:key="{{ $itemIndex }}" @click="toggleItem('{{ $items->id }}')"
                            ...
                            <td scope="row" wire:click.stop="edit('{{ $items->id }}')">{{ $items->type }}</td>
