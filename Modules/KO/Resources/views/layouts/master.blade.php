...
<html lang="en">
    ...
    <body x-data>
        ...
        <script type="text/javascript">          
          ...
          window.addEventListener('toast-loader',({detail:{type,message, action}})=>{
        ...
        @include('ko::layouts.partials.preview-modal')

        <script type="text/javascript">
            function previewBlobFile(id, fileName, type = 'attachment', field = null, path = null) {
                ...
                let routeUrl = "{{ route('ko::attachments.sas-uri', ['id' => ':id']) }}".replace(':id', id || 0) + '?type=' + type;
                if (field) {
                ...
                fetch(routeUrl)
                    ...
                    .then(data => {
                        ...
                        if (ext === 'pdf') {
                            let previewUrl = "{{ route('ko::attachments.preview', ['id' => ':id']) }}".replace(':id', id || 0) + '?type=' + type;
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Global Interception for previewable files in KO module
                document.body.addEventListener('click', function(e) {
                    ...
                    const isPreviewable = previewableExtensions.some(ext => urlLower.endsWith(ext) || urlLower.includes(ext + '?') || urlLower.includes(ext + '#'));
                    
                    if (!isPreviewable) return;
                    
                    const isKoStorage = href.includes('/storage/ko/') || 
                                        href.includes('/storage/commissioning/') || 
                                        href.includes('/storage/issue-report/') || 
                                        href.includes('/storage/request-qr/');
                    const isAzureBlob = href.includes('blob.core.windows.net');
                    
                    if (isKoStorage || isAzureBlob) {
                        e.preventDefault();
                        ...
                        previewBlobFile(null, fileName, 'attachment', null, href);
                    }
                });
            });
        </script>
