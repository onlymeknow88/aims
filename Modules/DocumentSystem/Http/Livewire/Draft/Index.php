...
namespace Modules\DocumentSystem\Http\Livewire\Draft;
...
class Index extends Component
{
    ...
    public function mount()
    {
        ...
        $this->fieldCompany = Company::select('companies.id', 'companies.company_name')
            ...
            ->where('document_system_documents.status', Document::DRAFT)
            ->groupBy('companies.id', 'companies.company_name')
            ->get();

        $this->fieldDepartment = Document::where('status', Document::DRAFT)
            ->get()
        ...
        $this->fieldDocumentTypes = Document::where('status', Document::DRAFT)
        ...
        try {
            ...
            for ($a = 0; $a < count($ids); $a++) {
                ...
                if ($data->status == Document::ACTIVE) {
                    ...
                    return redirect()->route('document-systems::document-systems.draft');
                }
    ...
    {
        return view('documentsystem::livewire.draft.index')->layout('documentsystem::layouts.app');
    }
}
