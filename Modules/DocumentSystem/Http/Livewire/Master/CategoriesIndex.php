...
class CategoriesIndex extends Component
{
    ...
    public function saveData($id = null)
    {
        $validate = $this->validate($this->validationRules($id), [
            ...
            'index.required' => trans('global.index_required'),
        ...
        try {
            ...
            $this->flash('success', trans('global.success_add_category'), [
                'position' => 'top-end',
                'timer' => 3000,
                'toast' => true,
            ]);

            return redirect()->route('document-systems::master.categories.index');
        } catch (\Throwable $th) {
    ...
    {
        return Category::with('module:id,name')->orderBy('index', 'asc')->paginate($this->limit);
    }
    ...
    public function submitDelete($id)
    {
        ...
        if ($data->delete()) {
            $this->flash('success', trans('global.success_delete_category'), [
                'position' => 'top-end',
                'timer' => 3000,
                'toast' => true,
            ]);
