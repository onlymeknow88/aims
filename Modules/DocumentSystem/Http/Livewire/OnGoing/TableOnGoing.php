...
namespace Modules\DocumentSystem\Http\Livewire\OnGoing;
...
class TableOnGoing extends Component
{
    use WithPagination;
    use LivewireAlert;
    ...
    public function mount()
    {
        ...
        $this->fieldCompany = Company::select('companies.id', 'companies.company_name')
            ...
            ->whereNotIn('document_system_documents.status', [Document::ACTIVE, Document::EXPIRED, Document::DRAFT, Document::OBSOLATE])
            ->groupBy('companies.id', 'companies.company_name')
            ->get();

        $this->fieldDepartment = Document::whereNotIn('status', [Document::ACTIVE, Document::EXPIRED, Document::DRAFT, Document::OBSOLATE])
            ->get()
        ...
        $this->fieldDocumentTypes = Document::whereNotIn('status', [Document::ACTIVE, Document::EXPIRED, Document::DRAFT, Document::OBSOLATE])
    ...
    public function getListingsProperty(): LengthAwarePaginator
    {
        try {
            return Document::with(['attachments' => fn($q) => $q->where('status', 1), 'department.company', 'user', 'mapping.category.module'])
                ...
                ->whereNotIn('status', [Document::ACTIVE, Document::EXPIRED, Document::DRAFT, Document::OBSOLATE])
    ...
    public function submitDelete()
    {
        ...
        try {
            ...
            $this->flash('error', trans('global.success_delete_document'), [
                'position' => 'top-end',
                'timer' => 3000,
                'toast' => true,
            ]);
    ...
    {
        return view('documentsystem::livewire.on-going.table-on-going')->layout('documentsystem::layouts.app');
    }
}
