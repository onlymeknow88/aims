...
class AddNewDocument extends Component
{
    ...
    public function mount($id = null)
    {
        ...
        // ── Edit Mode: pre-fill form with existing document data ──
        if ($id) {
            $this->id_maker = $id;
            $doc = Document::with(['attachments' => fn($q) => $q->where('status', 1), 'department', 'peoples'])->find($id);

            if ($doc) {
                // Owner info
                $this->department_code_id = $doc->department_code_id;
                ...
                if ($dept) {
                    ...
                    // Build departments dropdown for the selected company
                    $company = $this->companies->find($dept->company_id);
                    if ($company) {
                        $this->company_code = $company->document_code;
                        $new_departs = [];
                        foreach ($company->departments as $d) {
                            foreach ($d->codes as $dc) {
                                $new_departs[] = ['id' => $dc->id, 'name' => $d->name . ' - ' . $dc->code];
                            }
                        }
                ...
                $this->sop_number    = $doc->sop_number;
                $this->win_number    = $doc->sop_add_win;
                $this->form_number   = $doc->sop_add_form;
                $this->document_number = $doc->document_number;
                $this->title         = $doc->title;
                $this->description   = $doc->description;
                $this->doc_created   = $doc->doc_created;

                // Populate categories list based on preselected module_id
                if ($this->module_id) {
                    $selected_module = $this->modules->find($this->module_id);
                    if ($selected_module) {
                        $this->has_document_number = $selected_module->has_document_number;
                        $this->categories = $selected_module->categories;
                        $this->showDocumentType = ($selected_module->name == 'Dokumentasi');
                    }
                }

                // Populate mappings list based on preselected category_id
                if ($this->category_id) {
                    $selected_category = collect($this->categories)->firstWhere('id', $this->category_id);
                    if ($selected_category) {
                        $this->mapping = $selected_category->mappings;
                    }
                }

                // Populate PJ (pics) list and department configs based on department_id
                if ($this->department_id) {
                    ...
                    if ($dept_code_data) {
                        ...
                        $this->pics = Cache::remember('peoples', 600, function () use ($dept_code_data) {
                            return User::select('id', 'name', 'email')
                                ->where('department_id', $dept_code_data->department_id)
                                ->get();
                        });

                        $department = collect($this->departments)->firstWhere('id', $this->department_id);
                        if ($department) {
                ...
                // Populate invited people
                $this->invitedPeople = $doc->peoples->pluck('user_id')->toArray();
                ...
                foreach ($doc->attachments as $att) {
                    ...
                    $this->tmp[] = [
                        ...
                        'name'       => $att->file_name,
                        'ext'        => $att->file_type,
                        'ext_icon'   => $service->define_file_icon($att->file_type),
                        'size'       => $att->file_size,
                        'img_preview' => null,
                    ];
                }
    ...
    {
        return view('documentsystem::livewire.maker.add-new-document')
    ...
    public function saveData($status = 2)
    {
        ...
        try {
            ...
            if ($this->id_maker) {
                 ...
                 * Then change status to 3 / ROOTING REVIEW if user clicked 'Submit for Review' ($status = 1)
                 * Otherwise, retain PREPARE_ROOTING_REVIEW if user clicked 'Save as Draft' ($status = 2)
                 */
                if ($document->status == Document::PREPARE_ROOTING_REVIEW) {
                    if ($status == 1) {
