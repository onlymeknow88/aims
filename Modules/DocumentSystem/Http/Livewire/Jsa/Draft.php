...
class Draft extends Component
{
    ...
    public function mount()
    {
        ...
        $this->fieldCompany = Company::select('companies.id', 'companies.company_name')
            ...
            ->whereIn('jsa_documents.status', [JsaDocument::DRAFT])
            ->groupBy('companies.id', 'companies.company_name')
            ->get();

        $this->fieldDepartment = JsaDocument::whereIn('status', [JsaDocument::DRAFT])
            ->get()
    ...
    public function getListingsProperty(): LengthAwarePaginator
    {
        try {
            return JsaDocument::when(!empty($this->sortSelected), function ($q) {
                ...
                ->where(function ($q) {
                    $q->where('status', JsaDocument::DRAFT);
                })
    ...
    public function export()
    {
        ...
        return $service->export($this->itemSelected, 'draft');
    }
    ...
    {
        return view('documentsystem::livewire.jsa.draft')->layout('documentsystem::layouts.app');
    }
}
