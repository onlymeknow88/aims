...
class Active extends Component
{
    ...
    public function mount()
    {
        ...
        $this->fieldCompany = Company::select('companies.id', 'companies.company_name')
            ...
            ->whereIn('jsa_documents.status', [JsaDocument::ACTIVE, JsaDocument::EXPIRED])
            ->groupBy('companies.id', 'companies.company_name')
            ->get();

        $this->fieldDepartment = JsaDocument::whereIn('status', [JsaDocument::ACTIVE, JsaDocument::EXPIRED])
            ->get()
    ...
    public function getListingsProperty(): LengthAwarePaginator
    {
        try {
            return JsaDocument::when(!empty($this->sortSelected), function ($q) {
                ...
                ->where(function ($q) {
                    $q->where('status', JsaDocument::ACTIVE)
                        ->orWhere('status', JsaDocument::EXPIRED);
                })
    ...
    {
        return view('documentsystem::livewire.jsa.active')->layout('documentsystem::layouts.app');
    }
}
