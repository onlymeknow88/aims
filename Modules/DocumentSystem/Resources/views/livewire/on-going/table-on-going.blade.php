            ...
            <div x-data="{ 
                ...
                <div class="toolbar-tables border-top border-bottom d-flex justify-content-between p-2 sticky-top">
                    ...
                    <div class="toolbar-left d-flex align-items-center">
                        ...
                        @can('Document System - Delete Document')
                            <a type="button" 
                                                                    ...
                                                                    @if (isset($item))
                                                                        <div class="form-check">
                                                                            <input
                                                                                ...
                                                                                type="checkbox" value=""
                                                                                id="department_id"
                                                                                wire:click="sortCheck('department_id', '{{ optional($item)->id }}')">

                                                                            <label class="form-check-label fw-normal"
                                                                                for="flexCheckDefault">{{ optional($item)->name }}</label>
                                                                            </input>
                                                                    @endif
                                                                @endforeach
                                                            </div>
                            ...
                            <tbody>
                                @foreach ($this->listings as $itemIndex => $items)
                                    <tr wire:key="row-{{ $items->id }}"
                                        ...
                                        <td class="title">
                                            <a
                                                href="{{ route('document-systems::detail-maker', ['id' => $items->id, 'type' => 'ongoing']) }}" @click.stop>
                                                {{ $items->department->company->company_name ?? null }}
                                            </a>
                                        </td>
                                        <td>
                                            {{ $items->department->name ?? null }}
                                        </td>
                                        <td>
                                            {{-- <span>
                                                ...
                                                <ol>
                                                    {{-- @foreach ($items->attachments as $attachment)
                                                        <li>
                                                            <a href="javascript:void(0)"
                                                                onclick="event.stopPropagation(); previewBlobFile('{{ $attachment->id }}', '{{ $attachment->file_name }}', 'document')"
                                                                class="d-block">
                                                   ...
                                                   @foreach ($items->attachments as $attachment)
                                                        @if (str_starts_with($attachment->file_name, 'Final-'))
                                                            <li>
                                                                <a href="javascript:void(0)" onclick="event.stopPropagation(); previewBlobFile('{{ $attachment->id }}', '{{ $attachment->file_name }}', 'document')" class="d-block" @click.stop>
                                                                    {{ $attachment->file_name }}
                                                                </a>
                                                            </li>
                                                        @endif
