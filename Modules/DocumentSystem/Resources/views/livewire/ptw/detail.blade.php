...
<div class="inner-content">
    ...
    <div class="header-detail-maker h-60px border d-flex gap-2 align-items-center px-3">
        ...
        @if (
            $detail->status == \Modules\DocumentSystem\Entities\JsaDocument::EXPIRED &&
                ...
                auth()->user()->can('Document System - Edit PTW'))
        ...
        @if (auth()->user()->can('Document System - Create PTW') ||
                auth()->user()->can('Document System - Edit PTW'))
            <a href="{{ route('document-systems::ptw.edit', ['id' => $id_maker]) }}"
    ...
    <div class="detail-maker-content d-flex">
                    ...
                    <div class="action-wrapper d-flex align-items-center justify-content-end gap-2">
                        ...
                        <div class="button-document" wire:loading.remove wire:target='confirmRooting'>
                            <button
                                ...
                                type="button"
                                wire:click.prevent="changeStatus({{ $detail->status == \Modules\DocumentSystem\Entities\PtwDocument::ACTIVE ? \Modules\DocumentSystem\Entities\PtwDocument::INACTIVE : \Modules\DocumentSystem\Entities\PtwDocument::ACTIVE }})">
                                @if ($detail->status == \Modules\DocumentSystem\Entities\PtwDocument::ACTIVE)
                                    Deactivate Document
                                @else
                                    Activate Document
                                @endif
        ...
        <div class="detail-right border-start border-1">
            ...
            @include('documentsystem::livewire.ptw.components.activity', [
...
@push('scripts')
    <script>
        ...
        //     });
        // });
        window.addEventListener('confirm-change-status', (detail) => {
            newSwal.fire({
                title: 'Are you sure?',
                ...
                preConfirm: function(result) {
                    if (result) {
                        return @this.call('submitChangeStatus', detail.detail.status);
                    }
                },
            });
        });
