...
class Extraction extends Component
{
    ...
    public function getExtractionsProperty(Request $request)
    {
        ...
        if(!Auth::user()->hasPermissionTo('KPP - Monitoring Ekstraksi')) {
            $company_id = Auth::user()->department->company->id;
            $obediences = KppObedience::where('company_id', $company_id);
            $extractions->whereIn('obedience_id', $obediences->pluck('id'));
        }

        return $extractions->latest()->paginate($this->limit);
    }
    ...
    {
        return view('kpp::livewire.extraction.extraction')->layout('kpp::layouts.app');
    }
}
