...
class Lists extends Component
{
    ...
    public $searchCcow, $searchCriteria, $searchCompanyBusinessType, $searchCompanyName, $searchCompanyAddress, $searchCompanySites, $searchCompanyBusinessLicenseNumber, $searchCompanyParent, $searchPIC;
    ...
    public function mount()
    {
        ...
        if (Auth::user()->can('CSMS - Bidding View Active')) {
            $b = Bidding::where('criteria', CsmsStatus::Bidding)
    ...
    public function getLabelListingsProperty()
    {
        return Bidding::when(!empty($this->sortSelected), function ($query) {
            ...
            ->when(!auth()->user()->can('CSMS - Bidding View Active'), function ($query) {
                $query->where('maker_id', Auth::user()->id);
            })
            ->where('criteria', CsmsStatus::Bidding)
            ->where('status', CsmsStatus::Approved)
            ->where('requested', CsmsStatus::Approved)
    ...
    {
        return view('csms::livewire.bidding.lists')->layout('csms::layouts.app');
    }
}
