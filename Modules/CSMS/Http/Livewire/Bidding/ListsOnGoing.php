...
class ListsOnGoing extends Component
{
    use WithPagination;
    use LivewireAlert;
    ...
    public function mount()
    {
        ...
        if (auth()->user()->can('CSMS - Bidding View On Going')) {
            $b = Bidding::where('criteria', CsmsStatus::Bidding)
                ->whereIn('status', [CsmsStatus::OnReviewOHS, CsmsStatus::OnReviewDHOHS, CsmsStatus::OnReviewKTT])
                ->whereIn('requested', [CsmsStatus::RequestedOHS, CsmsStatus::RequestedDHOHS, CsmsStatus::RequestedKTT, CsmsStatus::Rejected, CsmsStatus::Approved])
    ...
    public function getBiddingListingsProperty()
    {
        return Bidding::when(!empty($this->sortSelected), function ($query) {
            ...
            ->when(!auth()->user()->can('CSMS - Bidding View On Going'), function ($query) {
    ...
    {
        return view('csms::livewire.bidding.lists-on-going')->layout('csms::layouts.app');
    }
}
