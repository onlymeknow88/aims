...
class ListsDraft extends Component
{
    use WithPagination;
    use LivewireAlert;
    ...
    public function mount()
    {
        ...
        if (auth()->user()->can('CSMS - Bidding View Draft')) {
    ...
    public function getBiddingListingsProperty()
    {
        return Bidding::when(!empty($this->sortSelected), function ($query) {
            ...
            ->when(!auth()->user()->can('CSMS - Bidding View Draft'), function ($query) {
    ...
    {
        return view('csms::livewire.bidding.lists-draft')->layout('csms::layouts.app');
    }
}
