...
class Edit extends Component
{
    ...
    public function mount($id)
    {
        ...
        $this->risk_category = $this->bidding->risk_category;

        $question = json_decode($this->bidding->questionnaire, true);
        $this->company_nickname = $question['company_nickname'];
        $this->scope_of_business = $question['scope_of_business'];
        $this->contract_period = $question['contract_period'];
        $this->number_of_workers = $question['number_of_workers'];
        $this->number_of_spv_pop = $question['number_of_spv_pop'];
        $this->number_of_spv_pom = $question['number_of_spv_pom'];
        $this->number_of_spv_pou = $question['number_of_spv_pou'];
        $this->number_of_spv_imp_smkp = $question['number_of_spv_imp_smkp'];
        $this->number_of_spv_auditor_smkp = $question['number_of_spv_auditor_smkp'];
        $this->equipped_name = $question['equipped_name'];
        $this->equipped_position = $question['equipped_position'];
        $this->equipped_telephone = $question['equipped_telephone'];
        $this->equipped_email = $question['equipped_email'];


        $this->emit('refreshComponent');

        foreach ($this->ChecklistCsms as $value) {
            $check = $this->bidding->checklists->where('question_id', $value['id'])->first();
            if ($check) {
                $this->{'checklist_csms_value_' . $value['id']} = $check->value;
                $this->{'checklist_csms_file_' . $value['id']} = null;
                $this->{'checklist_csms_note_' . $value['id']} = $check->comment;
                $this->{'files_data_' . $value['id']} = [];
                $this->{'oldFiles_' . $value['id']} = $check->files;
            } else {
    ...
    public function save($published, $status)
    {
        try {
            ...
            $post_bidding->checklists()->delete();
            foreach ($this->ChecklistCsms as $value) {
    ...
    {
        return view('csms::livewire.renewal.edit')->extends('csms::layouts.no-header');
    }
}
