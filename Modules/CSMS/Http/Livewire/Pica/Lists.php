...
namespace Modules\CSMS\Http\Livewire\Pica;
...
class Lists extends Component
{
    ...
    public function getBiddingListingsProperty()
    {
        return Bidding::when(!empty($this->sortSelected), function ($query) {
            ...
            ->whereHas('checklists', function ($query) {
                $query->whereIn('value', ['N/A','Tidak']);
                // dd($query);
            })
            ->where('maker_id', Auth::user()->id)
            ->whereIn('criteria', [CsmsStatus::Bidding,CsmsStatus::PostBidding, CsmsStatus::Renewal])
            ->whereIn('status', [CsmsStatus::Open])
            // ->whereIn('status', [CsmsStatus::Approved])
            ->whereIn('requested', [CsmsStatus::Approved])
            ->where('published', CsmsStatus::Publish)
    ...
    {
        return view('csms::livewire.pica.lists')->layout('csms::layouts.app');
    }
}
