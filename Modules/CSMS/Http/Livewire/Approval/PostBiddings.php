...
namespace Modules\CSMS\Http\Livewire\Approval;
...
class PostBiddings extends Component
{

    use WithPagination;
    use LivewireAlert;

    public $bidding_status = [], $bidding_requested = [], $limit, $status, $criteria;
    public $countData;
    public $itemSelected = [];
    public $countSelected = 0;
    public $info = false;

    public $columns = ['CCOW', 'Kriteria', 'Jenis Badan Usaha', 'Nama Perusahaan', 'Alamat Perusahaan', 'Site Perusahaan', 'Nomor Ijin Badan Usaha', 'Perusahaan Induk', 'Nama Penanggung Jawab Bidder'];
    ...
    public function mount()
    {
        ...
        array_push($this->bidding_requested, CsmsStatus::Approved);

        array_push($this->bidding_status, CsmsStatus::OnReviewOHS);
        array_push($this->bidding_status, CsmsStatus::OnReviewDHOHS);
        array_push($this->bidding_status, CsmsStatus::OnReviewKTT);

        array_push($this->bidding_requested, CsmsStatus::Approved);
        array_push($this->bidding_requested, CsmsStatus::RequestedOHS);
        array_push($this->bidding_requested, CsmsStatus::RequestedDHOHS);
        array_push($this->bidding_requested, CsmsStatus::RequestedKTT);
        ...
        // if (Auth::user()->can('CSMS - Bidding Reviewer D/H OHS')) {
        //     array_push($this->bidding_status, CsmsStatus::OnReviewDHOHS);
        //     array_push($this->bidding_requested, CsmsStatus::RequestedDHOHS);
        // }

        // if (Auth::user()->can('CSMS - Bidding Reviewer KTT')) {
        //     array_push($this->bidding_status, CsmsStatus::OnReviewKTT);
        //     array_push($this->bidding_requested, CsmsStatus::RequestedKTT);
        // }

        // dd($this->bidding_status);
        $b = Bidding::
            // where('maker_id', auth()->user()->id)->
            whereIn('criteria', [CsmsStatus::PostBidding, CsmsStatus::Renewal, CsmsStatus::Inactive])
            ->whereIn('status', $this->bidding_status)
            ->whereIn('requested', $this->bidding_requested)
    ...
    {
        return view('csms::livewire.approval.post-biddings')->layout('csms::layouts.app');
    }
}
