...
namespace Modules\CSMS\Http\Livewire\MemoKTT;
...
class Edit extends Component
{
    ...
    {
        $this->memo = CsmsMemoKtt::find($id);
        $this->company = Company::find($this->memo->ccow_id);

        $this->memo_number = $this->memo->memo_number;
        $this->title = $this->memo->title;
        $this->ccow_id = $this->memo->ccow_id;
        $this->ktt_id = $this->company->user->name;
        $this->date = $this->memo->date;
        $this->date_inactive = $this->memo->date_inactive;
        $this->description = $this->memo->description;
        $this->status = $this->memo->status;

        foreach ($this->memo->files as $key => $file) {
    ...
    public function saved()
    {
        try {
            ...
            return redirect()->route('csms::memo.index');
        } catch (\Throwable $err) {
    ...
    {
        return view('csms::livewire.memo-k-t-t.edit')->extends('csms::layouts.no-header');
    }
}
