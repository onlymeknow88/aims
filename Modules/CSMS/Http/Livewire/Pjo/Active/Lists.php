...
class Lists extends Component
{
    ...
    public function mount()
    {
        ...
        $this->countData = CsmsPjo::get()
            // ->where('created_at', auth()->user()->employee->id)
            ->groupBy('company_id')
            ->count();

        $this->limit = $this->countData;

        $this->fieldCompany = CsmsPjo::where('status', CsmsStatus::Approved)
        ...
        $this->fieldCcow = CsmsPjo::whereHas('ccow', function ($query) {
    ...
    {
        return view('csms::livewire.pjo.active.lists')->layout('csms::layouts.app');
    }
}
