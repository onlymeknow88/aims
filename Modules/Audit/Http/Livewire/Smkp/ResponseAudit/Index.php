...
namespace Modules\Audit\Http\Livewire\Smkp\ResponseAudit;
...
class Index extends Component
{
    ...
    public function saveDoc()
    {
        ...
        if($this->doc){
            try {
                \DB::beginTransaction();
                $image = $this->doc->store('storage/public/smkp/' . $this->audit->id . "/response_audit");
                $this->audit->response_audits()->create(['original_name'=>$this->doc->getClientOriginalName(),'url' => $image, 'status' => SubBundleStatusEnum::SUBMITTED]);
                \DB::commit();
    ...
    public function delete($audit_id,$id){
        ...
        try{
            $response_audit = AuditResponseAudit::find($id);
