...
namespace Modules\Audit\Http\Livewire\Iso45001\CriteriaAudit;
...
class Index extends Component
{
    ...
    {
        $this->audit = Audit::with(['criteria_module.criteria.sub_criteria.children.children.children.sample_methods'])->findOrFail($id);
        ...
        // dd($this->audit->criteria_module->criteria[2]);
    ...
    public function goTo($criteria_id)
    {
        ...
        return redirect()->route('audit::iso45001.detail.criteria-audit.detail', ['id' => $this->audit->id, 'criteria_id' => $criteria_id]);
    }

    public function render()
    {
        if (\Auth::user()->hasPermissionTo( 'Audit - Detail SMKP Criteria Audit')) {
            return view('audit::livewire.criteria-audit.index-iso',[
                'category'=>'iso45001'
    ...
    public function generateXLS($id){
        $this->mount($id);
        $array = Excel::toArray(new AuditCriteriaImport, storage_path('KriteriaAudit.xlsx'))[0];
        $audit_criteria = [];
        $criteria = $this->smkp->criteria_module->criteria;
