...
namespace Modules\Audit\Http\Livewire\Iso45001\ImplementationSchedule;
...
class Index extends Component
{
    ...
    protected function getAudit($id): void
    {
        ...
        // dd($this->implementationActivity->details[0]->schedules[0]->sub_criteria[0]->parent);
        ...
        foreach ($moduleCriteria->criteria as $keyCriteria => $criterion):
            foreach ($criterion->sub_criteria as $keySubCriteria => $sub_criterion):
                ...
                foreach ($sub_criterion->children as $keyChildren => $child):
                    foreach ($child->children as $keySubChild => $subChild) {
                        if ($subChild->has_point && !$subChild->excluded) {
                            $selectArrays[$subChild->id] =  $sub_criterion->title.", ".$child->title." : ".$subChild->title;
                        }
                    }

                    if ($child->has_point && !$child->excluded) {
                        $selectArrays[$child->id] =  $sub_criterion->title.": ".$child->title;
                    }
                endforeach;
            endforeach;
