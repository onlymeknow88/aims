...
<div class="inner-content">
    @include('audit::livewire.layouts.breadcrumb',[
         'trees'=>[
            ['name'=>strtoupper($category).' List','url'=>route('audit::'.$category.'.index')],
            ['name'=>'Baru'],
        ]
  ])
    <div class="container">
            ...
            <div class="col-sm-8">
                <form wire:submit.prevent='store' id="form-audit-init" class="py-4 d-flex flex-column gap-5">
                    <div class="title-form text-center mb-3">
                        <h3> AUDIT {{strtoupper($category)}}</h3>
                    </div><!-- /.title-form -->
                    ...
                    <div class="content-form d-flex flex-column gap-3">
                        ...
                        @if($category == 'smk3')
                        <div class="row form-group">
                            <label for="smk3_level" class="col col-form-label">Tingkat Audit</label>
                            <div class="col-8">
                                <x-inputs.select2
                                    id="smk3_level"
                                    error="smk3_level"
                                    wire:model='smk3_level'
                                    style="width: 100%"
                                    placeholder="Tingkat Audit">
                                    <option value="1">Tingkat Awal</option>
                                    <option value="2">Tingkat Transisi</option>
                                    <option value="3">Tingkat Lanjutan</option>
                                </x-inputs.select2>
                            </div>
                        </div>
                        @endif
                        <div class="row form-group">
                            ...
                            <div class="col-sm-4">
                                <x-inputs.datepicker placeholder="Start Date" wire:model="start_date" id="start_date" :error="'start_date'"></x-inputs.datepicker>
                            </div>
                            <div class="col-sm-4">
                                <x-inputs.datepicker placeholder="End Date" wire:model="end_date" id="end_date" :error="'end_date'"></x-inputs.datepicker>
                            </div>
                            <!-- <div class="col-8">
                                <x-inputs.text
                                    id="audit_time"
                    ...
                    <div class="footer-action mb-2 p-3">
                        <div class="action-wrapper d-flex align-items-center justify-content-end gap-2">
                            ...
                            <div class="form-group d-flex justify-content-end gap-2">
                                <a href="{{route('audit::'.$category.'.index')}}" class="btn btn-outline-secondary">Batal
                                </a>
                                <button
    ...
    <script>
        $(document).ready(function () {
            $('#audit_time').daterangepicker({
                ...
                @this.set('audit_time', start.format('DD/MM/YYYY').toString() + ' - ' + end.format('DD/MM/YYYY').toString());
            });
        });
    </script>
@endpush
