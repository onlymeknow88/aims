...
class Lists extends Component
{
    ...
    public $info = false;
    public $columns = ['Nomor Identitas', 'Tanggal Inspeksi', 'CCOW', 'Departemen', 'Section', 'Lokasi', 'Detail Lokasi', 'KTT', 'PJA', 'Keterangan Temuan', 'Status'];
    ...
    public function mount()
    {
        ...
        $last = KplhLabel::where('inspect_criteria', 'workplace')
            ->latest()
            ->first();
        ...
        $this->countData = KplhLabel::where('maker_id', auth()->user()->id)
            ->where('inspect_criteria', 'workplace')
            ->get()
            ->count();
    ...
    {
        if (Auth::user()->hasPermissionTo('KPLH - View List Workplace')) {
        // $dataTables = KplhLabel::where('inspect_criteria', 'workplace')
        //     ->with('inspection_data')
        //     ->orderByRaw("FIELD(status , 'draft', 'active', 'approved') ASC")
        //     ->orderBy('created_at', 'ASC');
        // $this->countData = $dataTables->count();
        // $data = $dataTables->paginate($this->limit);
        return view('kplh::livewire.weekly-workplace.lists')->layout('kplh::layouts.app');
