...
class Approval extends Component
{
    ...
    public $info = false;
    public $columns = ['Nomor Identitas', 'Tanggal Inspeksi', 'Jenis Inspeksi', 'CCOW', 'Departemen', 'Section', 'Lokasi', 'Detail Lokasi', 'KTT/PJO', 'PJA', 'Keterangan Temuan', 'Status','Pica Status'];
    ...
    public function mount()
    {
        ...
        $last = KplhLabel::where('status', '!=', 'draft')
            ->where('pja_id', auth()->user()->id)
            ->latest()
            ->first();

        $this->latestUpdate = 'Update on ' . Carbon::parse($last->date ?? null)->format('F d, Y . H:i A');
        ...
        $this->countData = KplhLabel::where('status', '!=', 'draft')
            ->where('pja_id', ((auth()->user()->areaManager) ? auth()->user()->areaManager->id : ''))
            ->count();
    ...
    public function getLabelListingsProperty()
    {
        return KplhLabel::when(!empty($this->sortSelected), function ($query) {
            $query->where(function ($query) {
                ...
                })->when($this->sortFieldSelected == 'pica_status', function ($query) {
                    array_key_exists('pica_status', $this->sortSelected) ? $query->whereIn('pica_status', $this->sortSelected['pica_status']) : '';
                });
            });
        })
            ...
            ->orderByRaw("FIELD(status,'active','approved') ASC")
            ->orderBy('date', 'ASC')
    ...
    {
        if (Auth::user()->hasPermissionTo('KPLH - Approval')) {
            return view('kplh::livewire.p-j-a.approval')->layout('kplh::layouts.app');
        } else {
            return abort(404);
        }
    }
