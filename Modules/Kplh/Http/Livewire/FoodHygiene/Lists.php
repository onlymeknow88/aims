...
class Lists extends Component
{
    ...
    public function mount()
    {
        ...
        $this->countData = KplhLabel::where('maker_id', auth()->user()->id)
            ->where('inspect_criteria', 'food-hygiene')
            ->get()
            ->count();
    ...
    {
        if (Auth::user()->hasPermissionTo('KPLH - View List Food Hygiene')) {
            return view('kplh::livewire.food-hygiene.lists')->layout('kplh::layouts.app');
        } else {
            return abort(404);
        }
    }
