...
class Lists extends Component
{
    ...
    public function mount()
    {
        ...
        $last = KplhLabel::where('inspect_criteria', 'area-jetty')
        ...
        $this->countData = KplhLabel::where('maker_id', auth()->user()->id)
            ->where('inspect_criteria', 'area-jetty')
            ->get()
            ->count();

        if ($this->countData) {
            $this->findMaker = KplhLabel::where('maker_id', auth()->user()->id)
                ->get()
                ->count();
        }
    }
    ...
    {
        if (Auth::user()->hasPermissionTo('KPLH - View List Area Jetty')) {
            return view('kplh::livewire.area-jetty.lists')->layout('kplh::layouts.app');
        } else {
            return abort(404);
        }
    }
