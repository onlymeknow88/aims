...
class Lists extends Component
{
    ...
    public function mount()
    {
        ...
        $last = KplhLabel::where('inspect_criteria', 'area-maintank')
            ->latest()
            ->first();
        ...
        $this->countData = KplhLabel::where('maker_id', auth()->user()->id)
            ->where('inspect_criteria', 'area-maintank')
            ->get()
            ->count();
    ...
    {
        if (Auth::user()->hasPermissionTo('KPLH - View List Area Maintank')) {
            return view('kplh::livewire.area-maintank.lists')->layout('kplh::layouts.app');
        } else {
            return abort(404);
        }
    }
