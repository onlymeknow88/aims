...
class Add extends Component
{
    ...
    public function mount()
    {
        ...
        $this->mcu_date = Carbon::now()->format('F d, Y');
    }

    public function checkIdNumber()
    {
        $e = Employee::with('companys', 'departments', 'user')->where('id_number', $this->employeeIdNumber)->first();

        if ($e) {
            $this->employeeDepartment = empty($e->departments->id) ? '' : $e->departments->id;
            $this->employeePosition = $e->position;
            $this->employeeNip = $e->number;
            $this->employeeName = $e->name;
            $this->employeeEmail = ($e->user) ? $e->user->email : '';
            $this->employeeBirthdate = Carbon::parse($e->date_of_birth)->format('d M Y');
            $this->employeeAge = Carbon::parse($e->date_of_birth)->age;
            $this->employeeGender = $e->gender;
            $this->employeeAddress = $e->address;
    ...
    public function updated($propertyName, $value)
    {
        ...
        if ($propertyName == 'weight' && $this->weight) {
            ...
            if ($this->height) {
                // $this->blood_cholesterol_total = $this->blood_hdl + $this->blood_ldl + (1 / 5 * $this->blood_tga);
                if ($this->employeeGender) {
                    $this->IdealFormula();
                }
            }
    ...
    public function save()
    {
        try {
            ...
            $employee = Employee::where('id_number', $this->employeeIdNumber)->first();
            if ($employee) {
                ...
                $user = User::where('email', $this->employeeEmail)->first();
                if (!$user) {
                    $u = new User;
                    $u->name = $this->employeeName;
                    $u->email = $this->employeeEmail;
                    $u->password = '$2y$10$wGUIAj.AT56sxspLjGgTq.mhGey4RvSPvswDIiIqVeWSlxIHUg2rG'; // password
                    $u->department()->associate($this->employeeDepartment);
                    $u->save();
                    ...
                    $e = Employee::create([
                        'user_id' => $usrId,
                        'id_number' => $this->employeeIdNumber,
                        'number' => $this->employeeNip,
                        'name' => $this->employeeName,
                        'department_id' => $this->employeeDepartment,
                        'position' => $this->employeePosition,
                        'date_of_birth' => $this->employeeBirthdate,
            ...
            // dd(Carbon::parse($this->mcu_date) . ' - ' . $mcu_exp_date);
            // $mcu_date = Carbon::createFromFormat('Y-m-d',$this->mcu_date);

            $mcudata = [
                'employee_id' => $this->employeeId,
                'staff_id' => auth()->user()->employee ? auth()->user()->employee->id : null,
            ...
            // dd(slugify($mcu_date->format('Y-m-d')));
            $create = MedicalHistory::create($mcudata);
    ...
    {
        if (Auth::user()->hasPermissionTo('MCU - Create MCU')) {
            return view('mcu::livewire.medical-staff.add', ['staff' => Employee::where('user_id', $this->user_id)->first(), 'employee' => Employee::all()->where('user_id', '!=', $this->user_id)])->extends('mcu::layouts.no-header');
        } else {
            return abort(404);
        }
    }
