...
class Lists extends Component
{
    ...
    public function getMcuDatasProperty()
    {
        return MedicalHistory::when(!empty($this->sortSelected), function ($query) {
            ...
            ->orderByRaw("FIELD(doctor_status_review , 'draft', 'In Review', 'Unfit', 'Fit With Recomendation', 'Curently Unfit', 'Fit') ASC")
    ...
    public function import()
    {
        ...
        $this->flash('success', 'Data MCU berhasil di import', [
            'position' => 'top-end',
            'timer' => 3000,
            'toast' => true,
        ], route('mcu::medical-staff'));
    ...
    public function export()
    {
        ...
        // $mcus = MedicalHistory::all()->pluck('id');
        // dd($mcus);
        return Excel::download(new MedicalHistoryExport($this->itemSelected), '' . $file_name . '.xlsx');
        // return Excel::download(new MedicalHistoryExport($mcus), '' . $file_name . '.xlsx');
    ...
    public function render()
    {
        if (Auth::user()->hasPermissionTo('MCU - View List MCU Medical Staff')) {
            return view('mcu::livewire.medical-staff.lists')->layout('mcu::layouts.app');
        } else {
            return abort(404);
        }
    }
