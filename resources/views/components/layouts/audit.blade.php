...
<body x-data>
  ...
  <script>
    ...
    @include('pica::layouts.partials.preview-modal')

    <script type="text/javascript">
        function previewBlobFile(id, fileName, type = 'notice-letter', field = null, path = null) {
            ...
            let routeUrl = "{{ route('audit::files.sas-uri', ['id' => ':id']) }}".replace(':id', id || 0) + '?type=' + type;
            if (field) {
            ...
            fetch(routeUrl)
                ...
                .then(data => {
                    ...
                    if (ext === 'pdf') {
                        let previewUrl = "{{ route('audit::files.preview', ['id' => ':id']) }}".replace(':id', id || 0) + '?type=' + type;
        ...
        function initPreviewScript() {
            ...
            // Global Interception for previewable files in Audit module
            document.addEventListener('click', function(e) {
                ...
                const hasAuditAttributes = anchor.hasAttribute('data-id') || anchor.classList.contains('previewable') || anchor.hasAttribute('data-preview');
                
                if (isAzureBlob || isStorageFile || hasAuditAttributes) {
                    e.preventDefault();
                    ...
                    const fileType = anchor.getAttribute('data-type') || 'notice-letter';
