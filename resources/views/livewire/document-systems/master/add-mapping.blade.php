                ...
                <form wire:submit.prevent='saveData' wire:ignore.self id="form-mapping">
                    ...
                    <div class="modal-body">
                        <div class="form-group mb-3">
                            ...
                            <x-inputs.select2 wire:model="category_id" error="category_id" id="category_id" parent="modalForm" placeholder="{{ trans('global.select_category') }}">

                                @foreach ($categories as $key => $category)
                                <option value="{{ $category['id'] }}">{{$category['name']}} - {{ $category->module->name }}</option>
                                @endforeach
